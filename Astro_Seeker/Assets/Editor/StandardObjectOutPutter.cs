...
public class StandardObjectOutPutter : MonoBehaviour
{
    [MenuItem("メニュー/オブジェクトデータ出力")]
    public static void OutputLocationData()
    {
        string fileName = EditorUtility.SaveFilePanel("出力ファイル", "", "", "loc");
        if (fileName == "")
        {
    ...
    private static bool WriteRecursiveData(BinaryWriter bw, GameObject gameObject, Vector3 offsetPos)
    {
        if (gameObject == null)
        {
            return false;
    ...
    private static bool WriteTransformData(BinaryWriter bw)
    {
        ...
        WriteRecursiveData(bw, topObject, new Vector3(0, 0, 0));
